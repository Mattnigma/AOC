...
#FUNCTION TO CHECK WHICH BOARDS HAVE WON
#RETURNS 0 UNLESS FINAL BOARD HAS WON
def losecheck(b,winners):
    for boardset in b:
        j=0
        for board in boardset:
            for row in board:
                if len(row)==5:
                    if j not in winners:
                        winners.append(j)
                    if len(winners)==len(b[0]):
                        return j
            j+=1
    return winners

#CHECK GUESSES TO SEE IF THEY MAKE A FINAL WIN
winners=[]
for guess in guesses:
    x=submitguess(guess,a,b1,b2)
    y=losecheck(x,winners)
    if y!=winners:
        winningguess=guess
        break
    else:
