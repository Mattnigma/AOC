...
def convert_vectors_to_points(vector_list):
    ...
    for vector in vector_list:
        j=0
        if vector[0][0]==vector[0][1]: #IF HORIZONTAL LINE
            while j <= abs(vector[1][0]-vector[1][1]):
                points_list.append([vector[0][0],min(vector[1][0],vector[1][1])+j])
                j+=1
        elif vector[1][0]==vector[1][1]: #IF VERTICAL LINE
            while j <= abs(vector[0][0]-vector[0][1]):
                points_list.append([min(vector[0][0],vector[0][1])+j,vector[1][0]])
                j+=1
        elif vector[0][0] + vector[1][0] - vector[0][1]- vector[1][1] != 0: #IF POSITIVE DIAGONAL LINE
            if vector[0][0]<vector[0][1]:
                k=0
            else:
                k=1
            while j <= abs(vector[0][0]-vector[0][1]):
                points_list.append([vector[0][k]+j , vector[1][k]+j])
                j+=1
        else: #IF NEGATIVE DIAGONAL LINE
            if vector[0][0]<vector[0][1]:
                k=0
            else:
                k=1
